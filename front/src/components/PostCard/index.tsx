...
const PostCard = memo(({ postData, postIndex }: Props) => {
  ...
  const liked = useMemo(
    () => info && postData.Likers && postData.Likers.find((v) => v.id === info.id),
    [info && info.id, postData && postData.Likers],
  );
  ...
  return (
    <CardWrapper>
      <Card
        ...
        actions={[
          liked ? (
            <HeartTwoTone twoToneColor="#eb2f96" key="heart" onClick={onClickLike} />
          ) : (
